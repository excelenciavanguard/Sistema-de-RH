...
const sources = [["RioVagas", 185, 38, "#0f766e"], ["Gmail", 131, 27, "#3b82f6"], ["Indeed", 97, 20, "#8b5cf6"], ["WhatsApp", 73, 15, "#f59e0b"]];
...
const stageColors = ["#3b82f6", "#8b5cf6", "#06b6d4", "#f59e0b", "#f97316", "#6366f1", "#16a34a"];
const stageColorFor = (stage) => stageColors[stages.findIndex(([label]) => label === stage)] ?? "#235347";
...
export function ReportsScreen() {
  ...
  const stagePieItems = stages.map(([label, count], index) => [label, Math.round(count / 486 * 100), Math.round(count / 486 * 100), stageColors[index]]);
  ...
  return <main className="screen-workspace miro-screen reports-reference-screen">
    ...
    <section className="report-kpi-grid report-primary-kpis">{[[FileText, "Currículos recebidos", data.resumes, "no período selecionado", "blue"], [ArrowUpRight, "Maior fonte", "RioVagas", `${data.topSource} currículos · 38% do total`, "violet"], [BriefcaseBusiness, "Vagas abertas", data.openVacancies, "com processos em andamento", "amber"]].map(([Icon, label, value, note, tone]) => <article className={`report-kpi-${tone}`} key={label}><span><Icon /></span><div><b>{label}</b><strong>{value}</strong><small>{note}</small></div></article>)}</section>
