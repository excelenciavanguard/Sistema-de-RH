import { ArrowRight, Check, Clock, FileText, FunnelSimple, MagnifyingGlass, Plus, UserCircle } from "@phosphor-icons/react";
import { ScreenHeader } from "../components/ScreenHeader.jsx";
import { requisitions } from "../mockWorkspaceData.js";
import { ROUTES } from "../navigation.js";

export function RequisitionsScreen({ onNavigate }) {
  return (
    <main className="screen-workspace">
      <ScreenHeader title="Requisições de vaga" description="Operações solicita a necessidade; a Diretoria decide; o RH transforma a aprovação em vaga." actions={<button className="primary-action" type="button" onClick={() => onNavigate(ROUTES.requisitionCreate)}><Plus size={18} /> Nova requisição</button>} />
      <section className="responsibility-flow" aria-label="Fluxo de responsabilidade">
        <div className="flow-step active"><span>1</span><div><strong>Operações cria</strong><small>Informa posto, função, quantidade e motivo.</small></div></div>
        <ArrowRight size={18} />
        <div className="flow-step"><span>2</span><div><strong>Diretoria aprova</strong><small>Aprova, devolve para ajuste ou rejeita.</small></div></div>
        <ArrowRight size={18} />
        <div className="flow-step"><span>3</span><div><strong>RH publica a vaga</strong><small>Completa requisitos, perguntas e etapas.</small></div></div>
      </section>
      <section className="surface-panel data-panel">
        ...
        <div className="request-table" role="table" aria-label="Lista de requisições">
          ...
          {requisitions.map((item) => <div className="request-row" role="row" key={item.code}>
            ...
            <span><button className="table-action" type="button" onClick={() => item.tone === "warning" ? onNavigate(ROUTES.approvals) : undefined}>{item.tone === "warning" ? "Analisar" : "Ver detalhes"} <ArrowRight size={15} /></button></span>
          </div>)}
        </div>
