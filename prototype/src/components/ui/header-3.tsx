      ...
      <div className="alpha-header-bar">
        <div className="mx-auto flex h-[68px] w-full max-w-[1820px] items-stretch gap-5 px-5 lg:px-8">
          ...
          <div className="ml-auto flex shrink-0 items-center gap-1.5">
            <label className="alpha-header-search h-10 w-[320px] items-center gap-2 rounded-lg border border-[#DFF3E4] bg-white px-3 text-[#235347] transition-colors focus-within:border-[#235347] focus-within:ring-2 focus-within:ring-[#235347]/10">
              <Search className="size-4 shrink-0" />
              <input type="search" aria-label="Buscar no Alpha RH" placeholder="Buscar pessoas, vagas ou menus" className="min-w-0 flex-1 bg-transparent text-[11px] text-[#163832] outline-none placeholder:text-[#235347]" />
              <kbd className="rounded border border-[#DFF3E4] bg-white px-1.5 py-0.5 text-[9px] font-semibold text-[#235347]">⌘ K</kbd>
            </label>
            ...
            <span className="hidden h-8 w-px bg-[#DFF3E4] xl:block" aria-hidden="true" />
            <button className="alpha-profile-button ml-0.5 flex h-12 items-center gap-2.5 border-0 bg-transparent px-1 text-left outline-none focus-visible:ring-2 focus-visible:ring-[#235347]/30" type="button" aria-label="Abrir perfil de Lucas">
              <span className="grid size-9 place-items-center rounded-full bg-[#0B2B26] text-[11px] font-extrabold text-white">LS</span>
              <span className="hidden flex-col 2xl:flex"><strong>Lucas</strong><small>RH Global</small></span>
              <ChevronDown className="hidden size-3.5 text-[#235347] 2xl:block" />
            </button>
            <Button size="icon" variant="outline" onClick={() => setMobileOpen((current) => !current)} className="alpha-mobile-toggle size-10 rounded-xl border-[#DFF3E4] bg-white text-[#163832] hover:bg-[#EDF6EF]" aria-expanded={mobileOpen} aria-controls="mobile-navigation" aria-label="Alternar menu">
...
function MobileMenu({ open, route }: { open: boolean; route: string }) {
  ...
  return createPortal(
    <div id="mobile-navigation" className="fixed inset-x-0 bottom-0 top-[68px] z-40 overflow-y-auto bg-[#EDF6EF] p-4 lg:hidden">
      <label className="flex h-11 items-center gap-2 rounded-xl border border-[#DFF3E4] bg-white px-3 text-[#235347] shadow-sm">
      ...
      <a href="#/inicio" className={cn('mt-3 flex items-center gap-3 rounded-xl bg-white p-3 text-sm font-bold text-[#0B2B26] shadow-sm', route === '/inicio' && 'ring-2 ring-[#235347]/20')}>
        <span className="grid size-9 place-items-center rounded-lg bg-[#EDF6EF] text-[#235347]"><Home className="size-[17px]" /></span>Início
      </a>
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        {menuGroups.map((group) => (
          <section key={group.label} className="rounded-2xl border border-[#DFF3E4] bg-white p-3 shadow-[0_8px_24px_rgba(20,54,84,.06)]">
            <div className="px-1 pb-2">
              <h2 className="mt-1 text-sm font-extrabold text-[#0B2B26]">{group.label}</h2>
              <p className="mt-1 text-[9px] leading-4 text-[#235347]">{group.description}</p>
            </div>
            <div className="grid gap-1">
              {group.subMenus.flatMap((subMenu) => subMenu.items).map((item) => {
                const Icon = item.icon;
                const active = item.active?.(route) ?? false;
                return <a key={item.label} href={item.href} className={cn('flex items-center gap-2.5 rounded-xl p-2.5 text-xs font-bold text-[#163832]', active ? 'bg-[#EDF6EF] text-[#163832]' : 'hover:bg-[#EDF6EF]')}><span className="grid size-8 place-items-center rounded-lg bg-[#EDF6EF] text-[#235347]"><Icon className="size-4" /></span>{item.label}</a>;
              })}
            </div>
          </section>
        ))}
...
const AlphaMark = (props: React.ComponentProps<'svg'>) => (
  <svg viewBox="0 0 42 46" fill="none" aria-hidden="true" {...props}>
    <path d="M21 2 40 43H29.8L20.7 22.6 11.5 43H1L21 2Z" fill="#051F20" />
    <path d="M21 2 40 43 24.7 29.4 20.7 22.6 11.5 43H1L21 2Z" fill="#051F20" />
    <path d="m9.4 31.7 11.3-9.1 5.1 11.5-16.4-2.4Z" fill="#DFF3E4" />
  </svg>
);
